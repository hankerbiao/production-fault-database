...
func viewFilters(q url.Values) store.ViewFilters {
	return store.ViewFilters{
		Keyword: query(q, "keyword"), From: query(q, "from"), To: query(q, "to"), DateFrom: query(q, "dateFrom"), DateTo: query(q, "dateTo"), TimeField: query(q, "timeField"), StationCode: query(q, "stationCode"),
		...
		HeadOrder: query(q, "headOrder"), ItemOrder: query(q, "itemOrder"), HeadSN: query(q, "headSn"), ItemSN: query(q, "itemSn"), MaterialCode: query(q, "materialCode"), OrderType: query(q, "orderType"), MissingSalesOrder: parseBool(q.Get("missingSalesOrder")),
