...
const emptyFaultStats = { total: 0, withError: 0, withRepairPerson: 0, salesOrders: 0, productionOrders: 0, hostBarcodes: 0, missingSalesOrder: 0, missingProductionOrder: 0, dataStartDate: '', dataEndDate: '', latestSyncedAt: '' };
...
const emptyViewStats = { total: 0, salesOrders: 0, productionOrders: 0, missingSalesOrder: 0, missingProductionOrder: 0, dataStartDate: '', dataEndDate: '', latestSyncedAt: '' };
const viewConfigs = {
...
function ViewDashboard({ config, modelOptions, setConnected, setRefreshing, refreshToken }) {
  ...
  const isBOMView = config.id === 'ZSGV_ZSD124';
  return <main><Hero title={config.title} detail={config.detail} meta={[{ label: bilingual ? '数据来源 / Data source' : '数据来源', value: config.source }, { label: bilingual ? '数据时间区间 / Data period' : '数据时间区间', value: formatDateRange(stats.dataStartDate, stats.dataEndDate) }, { label: bilingual ? '最新数据时间 / Latest data time' : '最新数据时间', value: formatDate(stats.latestSyncedAt) }]} />
    <section className="stats order-stats"><Stat icon={<Boxes />} label={bilingual ? '记录总数 / Total records' : '记录总数'} value={formatNumber(stats.total)} tone="blue" />{isBOMView && <Stat icon={<ClipboardList />} label="生产订单数量（去重）" value={formatNumber(stats.productionOrders)} tone="green" />}{isBOMView && <Stat icon={<ClipboardList />} label="销售订单数量（去重）" value={formatNumber(stats.salesOrders)} tone="orange" />}{(isBOMView || isStationView) && <Stat icon={<AlertTriangle />} label="销售订单为空" value={formatNumber(stats.missingSalesOrder)} tone="orange" />}{(isBOMView || isStationView) && <Stat icon={<AlertTriangle />} label="生产订单为空" value={formatNumber(stats.missingProductionOrder)} tone="red" />}<Stat icon={<Database />} label={bilingual ? '数据起始 / Data start' : '数据起始'} value={formatBusinessDate(stats.dataStartDate)} tone="green" /><Stat icon={<RefreshCw />} label={bilingual ? '数据截止 / Data end' : '数据截止'} value={formatBusinessDate(stats.dataEndDate)} tone="orange" /></section>
