...
import { Boxes, ClipboardList, PackageSearch, Search, SlidersHorizontal } from 'lucide-react';
...
export function OrdersPage({ modelOptions = [], customerIDOptions = [], setConnected, setRefreshing, refreshToken = 0 }) {
  ...
  return <main>
    ...
    <section className="workspace"><div className="section-head"><div><p className="eyebrow">SALES ORDERS</p><h2>订单明细</h2></div><div className="section-head-meta"><span className="record-count">共 {formatNumber(resource.total)} 条</span><span className="record-summary">机器数量汇总：{formatNumber(stats.machineQuantity)}</span></div></div>
      <form className="filters order-filters filter-form" aria-label="订单搜索条件" onSubmit={submit}><select aria-label="订单来源" value={resource.filters.source} onChange={event => update('source', event.target.value)}><option value="">全部来源</option><option value="SG">SG</option><option value="KK">KK</option></select><input className="filter-text" aria-label="生产订单" placeholder="生产订单" value={resource.filters.productionOrder} onChange={event => update('productionOrder', event.target.value)} /><input className="filter-text" aria-label="销售订单" placeholder="销售订单" value={resource.filters.salesOrder} onChange={event => update('salesOrder', event.target.value)} /><CustomerMultiSelect options={customerIDOptions} value={resource.filters.customer ? resource.filters.customer.split(',').filter(Boolean) : []} onChange={values => update('customer', values.join(','))} /><ModelSelect options={modelOptions} value={resource.filters.productModel} onChange={value => update('productModel', value)} placeholder="机型 / Product model" /><DateRangePicker from={resource.filters.dateFrom} to={resource.filters.dateTo} onChange={(dateFrom, dateTo) => resource.setFilters(previous => ({ ...previous, dateFrom, dateTo }))} label="订单时间周期" /><button className="filter-btn" type="submit" aria-label="搜索 / 筛选" disabled={resource.loading}><Search size={16} />{resource.loading ? '搜索中' : '搜索'}</button><button className="advanced-btn" type="button" onClick={() => setAdvancedOpen(value => !value)} aria-expanded={advancedOpen}><SlidersHorizontal size={16} />高级条件{advancedOpen ? '收起' : '展开'}</button><button className="reset-btn" type="button" onClick={reset} disabled={resource.loading}>重置</button><ExportButton exporting={exporting} onClick={exportData} />{advancedOpen && <div className="advanced-filters"><input className="filter-text" aria-label="关键字" placeholder="关键字：订单号、客户或机型" value={resource.filters.keyword} onChange={event => update('keyword', event.target.value)} /></div>}</form>
