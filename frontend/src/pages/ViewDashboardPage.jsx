...
import { AlertTriangle, Boxes, ClipboardList, Search, SlidersHorizontal } from 'lucide-react';
...
const emptyStats = { total: 0, salesOrders: 0, productionOrders: 0, missingSalesOrder: 0, missingProductionOrder: 0, missingProductionOrderDistinct: 0, is5000Count: 0, dataStartDate: '', dataEndDate: '', latestSyncedAt: '' };
const initialFilters = { keyword: '', productionOrder: '', salesOrder: '', productModel: '', from: '', to: '', dateFrom: '', dateTo: '', stationCode: '', sn: '', base: '', orderType: '', materialCode: '', headOrder: '', itemOrder: '', doaCode: '', status: '', judgment: '', customer: '', serviceOrder: '', changeType: '', partNumber: '', partSn: '', operator: '', needReturn: '', revoked: '', company5000: '' };
const filterLabels = { keyword: '关键字', productionOrder: '生产订单', salesOrder: '销售订单', productModel: '机型', from: '开始时间', to: '结束时间', stationCode: '工位', sn: '序列号', base: '基地', orderType: '订单类型', materialCode: '物料号', headOrder: '机头订单', itemOrder: '小刀订单', doaCode: 'DOA单号', status: '处理状态', judgment: '判定结果', customer: '客户', serviceOrder: '服务单', changeType: '操作类型', partNumber: '备件PN', partSn: '备件SN', operator: '操作人', needReturn: '需归还', revoked: '已撤销', company5000: '5000公司' };
...
export function ViewDashboardPage({ config, modelOptions = [], setConnected, setRefreshing, refreshToken = 0 }) {
  ...
  const removeFilter = key => resource.applyFilters({ ...resource.appliedFilters, [key]: initialFilters[key] || '' });
  ...
  const exportData = async (extraFilters = resource.appliedFilters, filename = `${config.title}.csv`) => { setExporting(true); try { await downloadCsv({ endpoint: `/api/views/${config.id}`, filters: extraFilters, filename, columns: columns.map(column => ({ key: column.key, label: column.label })) }); } finally { setExporting(false); } };
  ...
  return <main>
    ...
    {!resource.preview && <Stats className="view-stats"><Stat icon={<Boxes />} label={bilingual ? '记录总数 / Total records' : '记录总数'} value={formatNumber(resource.total)} tone="blue" />{(config.stats || []).map(item => <Stat key={item.key} icon={item.tone === 'red' || item.tone === 'orange' ? <AlertTriangle /> : <ClipboardList />} label={item.label} value={formatNumber(stats[item.key])} tone={item.tone} />)}</Stats>}
    <section className="workspace"><div className="section-head"><div><p className="eyebrow">{config.id}</p><h2>{bilingual ? `${config.title} 明细 / Details` : `${config.title}明细`}</h2></div><span className="record-count">{resource.preview ? `当前页 ${resource.items.length} 条` : `共 ${formatNumber(resource.total)} 条记录`}</span></div>
      <form className="filters dashboard-filters filter-form" aria-label={`${config.title}搜索条件`} onSubmit={submit}>
        <div className="filter-form-body">
          {config.showProductionOrder && <label className="filter-field"><span>{bilingual ? '生产订单 / Production order' : '生产订单'}</span><input className="filter-text" aria-label="生产订单" placeholder="AUFNR" value={resource.filters.productionOrder} onChange={event => update('productionOrder', event.target.value)} /></label>}
          {config.showSalesOrder && <label className="filter-field"><span>{bilingual ? '销售订单 / Sales order' : '销售订单'}</span><input className="filter-text" aria-label="销售订单" placeholder="VBELN" value={resource.filters.salesOrder} onChange={event => update('salesOrder', event.target.value)} /></label>}
          {config.showProductModel && <div className="filter-field"><span>{bilingual ? '产品层次 / Product hierarchy' : '机型'}</span><ModelSelect options={modelOptions} value={resource.filters.productModel} onChange={value => update('productModel', value)} placeholder={bilingual ? '产品层次 / Product hierarchy' : '机型 / Product model'} /></div>}
          {config.dateField && <div className="filter-field filter-field-date"><span>{bilingual ? `${config.dateLabel || '时间周期'} / Date period` : (config.dateLabel || '时间周期')}</span><DateRangePicker from={resource.filters.from} to={resource.filters.to} onChange={(from, to) => resource.setFilters(previous => ({ ...previous, from, to }))} label={bilingual ? `${config.dateLabel || '时间周期'} / Date period` : (config.dateLabel || '时间周期')} bilingual={bilingual} /></div>}
        </div>
        <div className="filter-actions"><button className="filter-btn" type="submit" aria-label={bilingual ? '搜索 / 筛选 / Search' : '搜索 / 筛选'} disabled={resource.loading}><Search size={16} />{resource.loading ? (bilingual ? '搜索中 / Searching' : '搜索中') : (bilingual ? '搜索 / Search' : '搜索')}</button><button className="advanced-btn" type="button" onClick={() => setAdvancedOpen(value => !value)} aria-expanded={advancedOpen}><SlidersHorizontal size={16} />{bilingual ? `高级条件${advancedOpen ? '收起' : '展开'} / More` : `高级条件${advancedOpen ? '收起' : '展开'}`}</button><button className="reset-btn" type="button" onClick={reset} disabled={resource.loading}>{bilingual ? '重置 / Reset' : '重置'}</button><ExportButton exporting={exporting} onClick={() => exportData()} bilingual={bilingual} />{config.allowMissingSalesOrderExport && <ExportButton exporting={exporting} onClick={() => exportData({ ...resource.appliedFilters, missingSalesOrder: true }, '订单BOM过账-销售订单为空.csv')} label="导出销售订单为空" title="导出销售订单为空的 BOM 数据" />}</div>
        {advancedOpen && <div className="advanced-filters"><div className="advanced-filters-head"><strong>{bilingual ? '更多筛选 / More filters' : '更多筛选'}</strong><span>{bilingual ? '按字段缩小结果范围 / Refine results' : '按字段缩小结果范围'}</span></div><div className="advanced-filters-grid"><input className="filter-text" aria-label={bilingual ? '关键字 / Keyword' : '关键字'} placeholder={bilingual ? '关键字 / Keyword' : '关键字'} value={resource.filters.keyword} onChange={event => update('keyword', event.target.value)} />{config.advancedFields?.map(field => field.type === 'select' ? <select key={field.key} aria-label={field.label} value={resource.filters[field.key] || ''} onChange={event => update(field.key, event.target.value)}><option value="">{field.placeholder}</option>{field.options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : field.type === 'checkbox' ? <label className="filter-checkbox" key={field.key}><input type="checkbox" aria-label={field.label} checked={resource.filters[field.key] === 'true'} onChange={event => update(field.key, event.target.checked ? 'true' : '')} /><span>{field.label}</span></label> : <input key={field.key} aria-label={field.placeholder} className="filter-text" placeholder={field.placeholder} value={resource.filters[field.key] || ''} onChange={event => update(field.key, event.target.value)} />)}</div></div>}
      </form>
