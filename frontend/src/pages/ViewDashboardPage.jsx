...
export function ViewDashboardPage({ config, modelOptions = [], setConnected, setRefreshing, refreshToken = 0 }) {
  ...
  return <main>
    ...
    <section className="workspace"><div className="section-head"><div><p className="eyebrow">{config.id}</p><h2>{bilingual ? `${config.title} 明细 / Details` : `${config.title}明细`}</h2></div><span className="record-count">{resource.preview ? `当前页 ${resource.items.length} 条` : `共 ${formatNumber(resource.total)} 条记录`}</span></div>
      <form className="filters dashboard-filters filter-form" aria-label={`${config.title}搜索条件`} onSubmit={submit}>{config.showProductionOrder && <input className="filter-text" aria-label="生产订单" placeholder="生产订单 / Production order" value={resource.filters.productionOrder} onChange={event => update('productionOrder', event.target.value)} />}{config.showSalesOrder && <input className="filter-text" aria-label="销售订单" placeholder="销售订单 / Sales order" value={resource.filters.salesOrder} onChange={event => update('salesOrder', event.target.value)} />}{config.showProductModel && <ModelSelect options={modelOptions} value={resource.filters.productModel} onChange={value => update('productModel', value)} placeholder={bilingual ? '产品层次 / Product hierarchy' : '机型 / Product model'} />}{config.dateField && <DateRangePicker from={resource.filters.from} to={resource.filters.to} onChange={(from, to) => resource.setFilters(previous => ({ ...previous, from, to }))} label={bilingual ? `${config.dateLabel || '时间周期'} / Date period` : (config.dateLabel || '时间周期')} bilingual={bilingual} />}<button className="filter-btn" type="submit" aria-label={bilingual ? '搜索 / 筛选 / Search' : '搜索 / 筛选'} disabled={resource.loading}><Search size={16} />{resource.loading ? (bilingual ? '搜索中 / Searching' : '搜索中') : (bilingual ? '搜索 / Search' : '搜索')}</button><button className="advanced-btn" type="button" onClick={() => setAdvancedOpen(value => !value)} aria-expanded={advancedOpen}><SlidersHorizontal size={16} />{bilingual ? `高级条件${advancedOpen ? '收起' : '展开'} / More` : `高级条件${advancedOpen ? '收起' : '展开'}`}</button><button className="reset-btn" type="button" onClick={reset} disabled={resource.loading}>{bilingual ? '重置 / Reset' : '重置'}</button><ExportButton exporting={exporting} onClick={() => exportData()} bilingual={bilingual} />{config.allowMissingSalesOrderExport && <ExportButton exporting={exporting} onClick={() => exportData({ ...resource.appliedFilters, missingSalesOrder: true }, '订单BOM过账-销售订单为空.csv')} label="导出销售订单为空" title="导出销售订单为空的 BOM 数据" />}{advancedOpen && <div className="advanced-filters"><input className="filter-text" aria-label={bilingual ? '关键字 / Keyword' : '关键字'} placeholder={bilingual ? '关键字 / Keyword' : '关键字'} value={resource.filters.keyword} onChange={event => update('keyword', event.target.value)} />{config.advancedFields?.map(field => field.type === 'select' ? <select key={field.key} aria-label={field.label} value={resource.filters[field.key] || ''} onChange={event => update(field.key, event.target.value)}><option value="">{field.placeholder}</option>{field.options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : field.type === 'checkbox' ? <label className="filter-checkbox" key={field.key}><input type="checkbox" checked={resource.filters[field.key] === 'true'} onChange={event => update(field.key, event.target.checked ? 'true' : '')} /><span>{field.label}</span></label> : <input key={field.key} aria-label={field.placeholder} className="filter-text" placeholder={field.placeholder} value={resource.filters[field.key] || ''} onChange={event => update(field.key, event.target.value)} />)}</div>}</form>
